    ...
    public static IServiceCollection AddApiRouters(this IServiceCollection services, params Assembly[] assemblies)
    {
        var targetAssemblies = assemblies.Length > 0
            ...
            : AppDomain.CurrentDomain.GetAssemblies()
                .Where(a => a.FullName?.StartsWith("SeaCode") == true)
                .ToList();
        ...
        {
            if (typeof(IGameRouterBase).IsAssignableFrom(type))
            {
                services.TryAddEnumerable(ServiceDescriptor.Transient(typeof(IGameRouterBase), type));
            }
