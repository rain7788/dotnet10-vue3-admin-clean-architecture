<!-- 标签页 -->
<template>
  <div v-if="showWorkTab" class="box-border flex-b w-full px-5 mb-3 select-none max-sm:px-[15px]" :class="[
    ...
    <div class="flex-1 min-w-0 overflow-hidden" ref="scrollRef">
      <ul class="float-left whitespace-nowrap !bg-transparent flex" :class="[tabStyle === 'tab-google' ? 'pl-1' : '']"
        ...
        <li class="art-card-xs inline-flex flex-cc h-8 mr-1.5 text-xs c-p hover:text-theme group" :class="[
          ...
          <span v-if="list.length > 1 && !item.fixedTab"
    ...
    <div class="flex">
      <div class="flex-cc art-card-xs relative top-0 size-8 leading-8 text-center c-p tad-200 hover:!bg-hover-color"
        :style="{
