  ...
  private checkComponents(
    ...
    routes.forEach((route) => {
      ...
      if (parentPath === '' && !hasExternalLink && !isIframe) {
        warnings.push(`一级菜单(${routePath}) 缺少 component，建议指向 ${RoutesAlias.Layout}`)
        return
      }

      // 非一级菜单：如果既不是外链、iframe，也没有子路由，则应该配置 component
      // 降级为警告而不是错误，让应用可以继续运行，访问时显示错误占位
      if (!hasExternalLink && !isIframe && !hasChildren) {
        warnings.push(`路由(${routePath}) 缺少 component 配置，访问时将显示错误页面`)
      }
