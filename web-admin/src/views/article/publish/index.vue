      ...
      <div class="max-w-250 mx-auto my-5">
        ...
        <div class="p-5 mt-5 art-card-xs">
          ...
          <ElForm>
            <ElFormItem label="封面">
              <div class="mt-2.5">
                <ElUpload :action="uploadImageUrl" :headers="uploadHeaders" :show-file-list="false"
                  ...
                  <div v-if="!cover"
                    ...
                    <ElIcon class="!text-xl !text-g-600">
                      <Plus />
                    </ElIcon>
...
import { fetchGetArticleTypes, fetchGetArticleDetail } from '@/api/article'
...
const getArticleTypes = async () => {
  try {
    const res = await fetchGetArticleTypes()
    if (res.code === 200) {
...
const getArticleDetail = async () => {
  try {
    const res = await fetchGetArticleDetail(route.query.id as string)
    if (res.code === ApiStatus.success) {
      const { title, blog_class, html_content } = res.data
