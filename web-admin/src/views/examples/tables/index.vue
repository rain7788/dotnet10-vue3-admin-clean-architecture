...
<template>
  <div class="flex flex-col gap-4 pb-5">
              ...
              <div class="flex flex-col gap-2">
                <div class="max-h-50 overflow-y-auto">
                  ...
                  <div v-else class="flex flex-col gap-1">
                    <div v-for="(log, index) in cacheDebugLogs" :key="index"
                      class="p-1.5 px-2 text-xs leading-[1.4] bg-g-200 border-l-1 border-g-400 rounded" :class="{
    ...
    <!-- 搜索区域 -->
    <ArtSearchBar ref="searchBarRef" v-model="searchFormState" :items="searchItems" :rules="rules" :is-expand="false"
    ...
    <ElCard class="flex-1 art-table-card" shadow="never" style="margin-top: 0">
      ...
      <ArtTableHeader v-model:columns="columnChecks" :loading="loading" @refresh="handleRefresh"
        ...
        <template #left>
          <ElSpace wrap>
            ...
            <!-- 导出导入功能 -->
            <ArtExcelExport :data="data as any" :columns="exportColumns as any" filename="用户数据" :auto-index="true"
      ...
      <ArtTable ref="tableRef" :loading="loading" :pagination="pagination" :data="data" :columns="columns"
        :height="computedTableHeight" empty-height="360px" @selection-change="handleSelectionChange"
    ...
    <ElCard shadow="never" class="art-card-xs">
      ...
      <div class="flex flex-col gap-6">
        ...
        <div class="p-4 bg-g-200 border-full-d rounded-lg">
          ...
          <div v-if="eventDemoEnabled && eventLogs.length > 0" class="p-3 mt-3 bg-g-200 border border-g-400 rounded-md">
            ...
            <div class="flex flex-col gap-1 max-h-50 overflow-y-auto">
              <div v-for="(log, index) in eventLogs.slice(0, 20)" :key="index"
        ...
        <div class="p-4 bg-g-200 border-full-d rounded-lg">
          ...
          <div class="flex flex-wrap gap-2 mb-3 last:mb-0">
            <ElSwitch v-model="tableConfig.fixedHeight" active-text="固定高度 (500px)" inactive-text="自适应高度" class="ml-2" />
          </div>
        </div>
...
import { fetchMockUserList } from '@/api/mock'
...
const {
  ...
  core: {
    apiFn: (params) => {
      ...
      return fetchMockUserList(params)
    },
    apiParams: {
      current: 1,
      size: 20,
